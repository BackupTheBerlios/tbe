...
public final class FTPHandler {
    ...
    public static void upload(FTPServer server, String localPath,
	    String remotePath) {
	if (client == null) {
	    connect(server);
	}
    ...
    public static void download(FTPServer server, String localPath,
	    String remotePath) {
	if (client == null) {
	    connect(server);
	}
    ...
    public static void upload(FTPServer server, ArrayList<String> localPaths,
	    ArrayList<String> remotePaths) {
	if (client == null) {
	    connect(server);
	}
	try {
    ...
    public static void download(FTPServer server, ArrayList<String> localPaths,
	    ArrayList<String> remotePaths) {
	if (client == null) {
	    connect(server);
	}
	try {
		...
		if (!remotePaths.get(j).contains("cvs")) {
		    String dir = localPaths.get(j).substring(0,
			    localPaths.get(j).lastIndexOf("/"));
			...
			System.out
				.println("Ordner konnte nicht erstellt werden!");
		    }
		}
	    }
	} catch (IOException e) {
