...
public final class XMLHandler {
	...
	class SaxHandler extends DefaultHandler {
		...
		try {
			...
			JOptionPane.showMessageDialog(TBE.getInstance()
				.getView(), "File not Found! ("
				+ file.getPath() + ")");
		    }
		} catch (Throwable t) {
		    t.printStackTrace();
	...
	class SaxHandler extends DefaultHandler {
		...
		if (actSport != null) {
			...
			shapes
				.add(new ItemType(atts.getValue("name"), atts
					.getValue("description"), actIcon,
					actPicture, Integer.parseInt(atts
						.getValue("maxSideWidth"))));
		    }
			...
			if (imgURL != null) {
			    Icon actIcon = new ImageIcon(imgURL);
			    arrows.add(new ItemType(atts.getValue("name"), atts
				    .getValue("description"), actIcon));
			}
		    }
	...
	if (board.getPath().equals("")) {
	    ...
	    if (filename != null
		    && !filename.getPath().toLowerCase().endsWith(".tbe")) {
	...
	try {
	    ...
	    format.setEncoding("iso-8859-1");
	    XMLOutputter out = new XMLOutputter(format);
	    java.io.FileWriter writer = new java.io.FileWriter(XMLHandler.class
		    .getResource("../config/tbe.config").getPath());
	    out.output(document, writer);
	    writer.flush();
	    writer.close();
	} catch (Exception ex) {
