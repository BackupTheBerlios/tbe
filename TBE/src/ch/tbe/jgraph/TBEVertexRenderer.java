...
public class TBEVertexRenderer extends JLabel implements CellViewRenderer, ImageObserver, Serializable {
    ...
    /**
         ...
         */
    public TBEVertexRenderer() {
	defaultForeground = UIManager.getColor("Tree.textForeground");
	defaultBackground = UIManager.getColor("Tree.textBackground");
    }

    /**
         * Configure and return the renderer component based on the passed in
         * cell. The value is typically set from messaging the graph with
         * <code>convertValueToString</code>. We recommend you check the
         * value's class and throw an illegal argument exception if it's not
         * correct.
         * 
         * @param graph
    ...
    /**
         * Hook for subclassers that is invoked when the installAttributes is
         * not called to reset all attributes to the defaults. <br>
	...
	try {
		...
		if (i != null) {
		    ...
		    g2d.rotate(Math.toRadians(TBEGraphConstants
			    .getRotation(map)));
		    g2d.translate(-getWidth() / 2, -getHeight() / 2);

		    g2d.drawImage(i, (int) (getWidth() / 2 - r.getWidth() / 2),
			    (int) (getHeight() / 2 - r.getHeight() / 2),
			    (int) r.getWidth(), (int) r.getHeight(), this);
		}
