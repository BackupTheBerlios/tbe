...
public class TBEBasicGraphUI extends BasicGraphUI {
	...
	public class TBEMouseHandler extends MouseAdapter implements MouseMotionListener, Serializable {
			...
			try {
				if (e != null && !e.isConsumed() && graph != null && graph.isEnabled()) {
					if (handler == marquee && marquee != null)
						...
						try {
							if (!mouseDown.equals(mouseUP) || (e.getButton() == 3 && graph.getSelectionCount() == 1 && graph.getSelectionCell() instanceof ArrowItem)) {
	...
	/**
   * Creates the listener responsible for calling the correct handlers based on
   * mouse events, and to select invidual cells.
   */
